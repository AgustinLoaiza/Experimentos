...
#include "ExperimentosProjectile.h"
...
const FName AExperimentosPawn::MoveForwardBinding("MoveForward");
const FName AExperimentosPawn::MoveRightBinding("MoveRight");
const FName AExperimentosPawn::FireForwardBinding("FireForward");
const FName AExperimentosPawn::FireRightBinding("FireRight");

AExperimentosPawn::AExperimentosPawn()
{	
...
void AExperimentosPawn::Tick(float DeltaSeconds)
{
...
//Inicalizacion de las funciones de recoger y soltar Armeria
void AExperimentosPawn::DropItemArmeria()
{
	if (Armeria->CurrentInventory.Num() == 0)
	{
		return;
	}
	AArmeria* Item = Armeria->CurrentInventory.Last();
	Armeria->RemoveFromInventory(Item);
	FVector ItemOrigin;
	FVector ItemBounds;
	Item->GetActorBounds(false, ItemOrigin, ItemBounds);
	FTransform PutDownLocation = GetTransform() + FTransform(RootComponent->GetForwardVector() * ItemBounds.GetMax());
	Item->SoltarArmeria(PutDownLocation);
}

void AExperimentosPawn::TakeItemArmeria(AArmeria* InventoryItem)
{
	InventoryItem->AgregarArmeria();
	Armeria->AddToInventory(InventoryItem);
}
...
void AExperimentosPawn::NotifyHit(class UPrimitiveComponent* MyComp, AActor* Other, class UPrimitiveComponent* OtherComp, bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	AArmeria* InventoryItemArmeria = Cast<AArmeria>(Other);
